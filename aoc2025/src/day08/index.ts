...
const part1 = (rawInput: string) => {
	...
	for (let x = 0; x < input.length; x++) {
		for (let y = x + 1; y < input.length; y++) {
			distances.push({ coords: [x, y], distance: distance(input[x], input[y]) });
		}
	}
	...
	for (let i = 0; i < 1000; i++) {
		const found = circuits.filter(circuit => circuit.includes(distances[i].coords[0]) || circuit.includes(distances[i].coords[1]));
		if (found.length === 0) {
			circuits.push(distances[i].coords);
		}
		if (found.length === 1) {
			const index = circuits.findIndex(circuit => circuit.join(",") === found[0].join(","));
			circuits[index].push(...distances[i].coords);
		}
		if (found.length === 2) {
			const index1 = circuits.findIndex(circuit => circuit.join(",") === found[0].join(","));
			const index2 = circuits.findIndex(circuit => circuit.join(",") === found[1].join(","));
